(function(w) {
	w.Calendar = function(opt) {
		...
		//document.querySelector(CSS selectors)方法返回文档中匹配指定 CSS 选择器的一个元素(仅仅返回匹配指定选择器的第一个元素)
		//追加日历div
		...
		document.getElementById("day").innerHTML = year + '-' + month + '-' + nowDay;
		document.getElementById("week").innerHTML = getWeek(dateObj);
		...
		content.addEventListener(evt.type, function(event) {
			if(event.target.tagName.toLowerCase() == "div" && event.target.nodeType == "1" && hasclass(event.target.className, "canChoose")) {
				...
				document.getElementById("day").innerHTML = getYear(dateObj) + '-' + getMonth(dateObj) + '-' + day;
		...
		prevmonth.addEventListener(evt.type, function() {
			mui.toast('暂无权限查看上月的签到记录，详情请联系管理员', {
				duration: 'short',
				type: 'div'
			})
			//			var ddm = null;
